    ...
    try {
      ...
      if (response.data && response.data.status === "success") {
        ...
        if (response.data.data && Array.isArray(response.data.data.products)) {
          ...
          console.log(
            "Found object in response.data.data, keys:",
            Object.keys(response.data.data)
          );
          // Try to extract products from any property that might contain them
          const possibleProductArrays = Object.values(
            response.data.data
          ).filter((val) => Array.isArray(val));
          if (possibleProductArrays.length > 0) {
            console.log(
              "Found potential product arrays:",
              possibleProductArrays.length
            );
        ...
        if (Array.isArray(response.data)) {
          ...
          console.log(
            "Found products array directly in response.data.products"
          );
