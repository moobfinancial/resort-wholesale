...
export default function ProductForm({
  ...
  const loadBulkPricing = async (productId: string) => {
    try {
      const response = await api.get<{ tiers?: BulkPricing[] }>(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      message.success(
        `Product ${initialValues ? "updated" : "created"} successfully!`
      );
  ...
  const handleBulkPricingUpdate = async (updatedTiers: any[]) => {
    ...
    console.log("updatedTiers in bulk price", updatedTiers);
    if (initialValues?.id) {
      try {
        await api.post<{ success: boolean }>(
          `products-bulk-pricing/${initialValues.id}/bulk-pricing`,
          {
            tiers: updatedTiers,
          }
        );
  ...
  const handleImageAnalysis = async (imageDataUrl: string) => {
    ...
    try {
      ...
      if (analysis.imageUrl) {
        ...
        const fileName =
          analysis.imageUrl.split("/").pop() || "analyzed_image.jpg";
  ...
  const normalizeSrc = (src: string) => {
    ...
    if (
      src.startsWith("/uploads/products/") ||
      src.startsWith("uploads/products/")
    ) {
      ...
      <Tabs activeKey={activeTab} onChange={setActiveTab}>
          ...
          >
            <Form.Item name="imageUrl" label="Product Image">
              <div style={{ marginBottom: "15px" }}>
                <Typography.Text>
                  Take a picture or upload an image of your product to
                  automatically analyze and fill product details
                </Typography.Text>
              </div>
              <div>
                <Upload
            ...
            <Form.Item
              ...
              tooltip={{
                title: (
                  <div>
                    <p>
                      <strong>Draft</strong>: Product is in development and not
                      visible to customers
                    </p>
                    <p>
                      <strong>Pending Review</strong>: Ready for admin review
                      before publishing
                    </p>
                    <p>
                      <strong>Approved</strong>: Product is approved but not yet
                      published
                    </p>
                    <p>
                      <strong>Published</strong>: Product is visible to
                      customers
                    </p>
                    <p>
            ...
            <Form.Item
              ...
              tooltip={{
                title:
                  "Featured products will appear on the homepage. Only products with status 'Approved' or 'Published' can be featured.",
                placement: "right",
              }}
            >
              <Switch
                disabled={!["APPROVED", "PUBLISHED"].includes(productStatus)}
              />
            </Form.Item>
        ...
        {initialValues?.id && (
          <Tabs.TabPane tab="Variants" key={`variants-${initialValues.id}`}>
            <div className="mb-4">
              <Typography.Text>
                Add size, color, or other variations of your product.
              </Typography.Text>
            </div>
