...
const ProductImageManager: React.FC<ProductImageManagerProps> = ({
  ...
  useEffect(() => {
    ...
    if (productId && token) {
      console.log(
        "ProductImageManager: Have productId and token, fetching images"
      );
      fetchImages();
    } else {
  ...
  const fetchImages = async () => {
    ...
    try {
      ...
      // Make API request
      const response = await api.get(`/product-images/${productId}/images`);
      ...
      if (response?.data) {
        ...
        if (Array.isArray(items)) {
          console.log("Setting images:", items);
          setImages(items);
        } else {
          console.warn("Unexpected response format:", response.data);
          message.error("Invalid image data received");
          setImages([]);
        }
      } else {
  ...
  //           console.log(
  //             "Setting images from direct array:",
  //             response.data.data
  //           );
  //           setImages(response.data.data);
  //         } else if (response.data.data?.items) {
  //           console.log(
  //             "Setting images from items array:",
        ...
        <img
          ...
          onError={(e) => {
            ...
            if (!target.src.includes("/images/products/placeholder.svg")) {
              console.log(
                "Image load error, trying fallback path:",
                target.src
              );
  ...
  const handleDelete = async (imageId: string) => {
    try {
      setLoading(true);
      // if (variantId) {
      //   endpoint = `/product-images/${productId}/variants/${variantId}/images/${editingImage.id}`;
      // } else {
      //   endpoint = `/product-images/${productId}/images/${editingImage.id}`;
      // }
      const endpoint = variantId
        ? `/product-images/${productId}/variants/${variantId}/images/${imageId}`
        : `/product-images/${productId}/images/${imageId}`;
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      if (editingImage) {
        console.log("Editing existing image:", editingImage);
        if (variantId) {
          endpoint = `/product-images/${productId}/variants/${variantId}/images/${editingImage.id}`;
        } else {
          endpoint = `/product-images/${productId}/images/${editingImage.id}`;
        ...
        if (variantId) {
          endpoint = `/product-images/${productId}/variants/${variantId}/images`;
        } else {
          endpoint = `/product-images/${productId}/images`;
        }
      }

      console.log(
        `Submitting image to endpoint: ${endpoint} with method: ${method}`
      );
      ...
      if (
        ...
        console.log("Successfully uploaded/updated image:", response.data.item);

        // Update UI and fetch the latest images
        await fetchImages();
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item
            ...
            label="Sort Order"
            tooltip={{
              title: "Lower numbers will appear first in the image gallery",
              placement: "topRight",
            }}
