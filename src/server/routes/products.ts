...
router.get('/categories', async (_, res) => {
  ...
  try {
    ...
    const categories = categoriesResponse.status === 'success' ? categoriesResponse.data.items : [];
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      const { items, total, page: currentPage, limit: itemsPerPage, totalPages } = result.data;
...
router.get('/:id/related', async (req, res) => {
  ...
  try {
    ...
    log('Related products:', result.status === 'success' ? result.data.items : []);
...
router.get('/:id/price', async (req, res) => {
  try {
    ...
    try {
      const bulkPriceResponse = await productService.getPriceForQuantity(id, quantity);
      // If bulk price is available, use it, otherwise use the product base price
      const finalPrice = bulkPriceResponse.status === 'success' && bulkPriceResponse.data.price !== null 
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const images = imagesResponse.status === 'success' ? imagesResponse.data.items : [];
    ...
    res.status(500).json({
      status: 'error',
      message: error instanceof Error ? error.message : 'Failed to get product details',
    });
  }
});
...
router.post('/:id/images', requireAdmin, (req: Request, res: Response) => {
  ...
  res.json({
    ...
    details: 'Please use /api/products/:productId/images from product-images.ts'
  });
});

// Forward image update requests
router.put('/:id/images/:imageId', requireAdmin, (req: Request, res: Response) => {
  console.log('PUT /:id/images/:imageId - Redirecting to product-images route');
  res.json({
    status: 'error',
    message: 'This endpoint has been moved',
    details: 'Please use /api/products/:productId/images/:imageId from product-images.ts'
  });
});

// Forward image delete requests
router.delete('/:id/images/:imageId', requireAdmin, (req: Request, res: Response) => {
  console.log('DELETE /:id/images/:imageId - Redirecting to product-images route');
  res.json({
    status: 'error',
    message: 'This endpoint has been moved',
    details: 'Please use /api/products/:productId/images/:imageId from product-images.ts'
...
router.get('/variants/:variantId/images', async (req: Request, res: Response) => {
...
router.post('/direct-test-variant/:productId', async (req: Request, res: Response) => {
  try {
    const { productId } = req.params;
    const variantData = req.body;
    
    console.log(`Creating direct test variant for product ${productId}:`, variantData);
