...
router.post("/:productId/bulk-pricing", requireAuth, async (req, res) => {
  try {
    ...
    const success = await bulkPricingService.updateProductTiers(
      productId,
      tiers
    );
...
router.put(
  "/:productId/bulk-pricing/tier/:tierId",
  requireAuth,
  async (req, res) => {
    try {
...
router.get("/:productId/price", async (req, res) => {
  try {
    ...
    const price = await bulkPricingService.calculatePriceForQuantity(
      productId,
      quantity
    );
