...
try {
  if (googleCloudConfig.geminiApiKey) {
    console.log(
      ...
      googleCloudConfig.geminiApiKey?.substring(0, 4) + "..."
    );
...
router.post("/analyze-image", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Processing uploaded image: ${req.file.originalname}, size: ${(
        req.file.size / 1024
      ).toFixed(2)}KB`
    );
    ...
    console.log(
      "Sending image to Gemini for analysis. API Key present:",
      !!googleCloudConfig.geminiApiKey
    );

    // Check if Gemini API key is available and model is initialized
    if (!googleCloudConfig.geminiApiKey || !model) {
      console.log(
        "No Gemini API key found or model not initialized. Skipping image analysis."
      );
    ...
    try {
        ...
        try {
          ...
          const name =
            lines
              ...
              ?.replace("Name:", "")
              .trim() || path.parse(fileName).name.replace(/[-_]/g, " ");
          const category =
            lines
              .find((l: string) => l.startsWith("Category:"))
              ?.replace("Category:", "")
              .trim() || "Other";
          const description =
            lines
      ...
      const uploadsProductsDir = path.join(
        process.cwd(),
        "uploads",
        "products"
      );
      ...
      console.log(
        `Image copied to public uploads directory: ${publicUploadsPath}`
      );
      ...
      console.log(
        `Image copied to public images directory: ${publicImagesPath}`
      );

      // Add the image URL to the result
      if (result) {
        // Use the correct path format for the frontend
        result.imageUrl = `/uploads/products/${filename}`;
    ...
    if (imagePath && fs.existsSync(imagePath)) {
      try {
        ...
        console.error(
          `Failed to delete temporary image file ${imagePath}:`,
          unlinkError
        );
      }
    }
...
router.post("/products", upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      console.log(
        "Processing base64 image data - length:",
        productData.imageUrl.length
      );

      // Extract the base64 data
      // const matches = productData.imageUrl.match(
      //   /^data:([A-Za-z-+\\/]+);base64,(.+)$/
      // );
      const matches = productData.imageUrl.match(
        /^data:([\w/+.-]+);base64,([\s\S]+)$/
      ...
      //   const base64Data = matches[2]; // Extracted base64 string
      ...
      productData.imageUrl === "placeholder" ||
      !productData.imageUrl
    ) {
      // Set the default placeholder image path
      console.log("Using placeholder image");
      productData.imageUrl = "/uploads/products/placeholder.svg";
    } else {
...
router.put("/products/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    if (!req.file && productData.imageUrl?.startsWith("data:")) {
      ...
      if (
        productData.imageUrl &&
        !productData.imageUrl.startsWith("/images/") &&
        !productData.imageUrl.startsWith("http")
      ) {
        const filename = productData.imageUrl.split("/").pop();
        if (filename) {
