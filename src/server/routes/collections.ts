    ...
    res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to get active collections",
    });
  }
});
  ...
  try {
    ...
    res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to get collection products",
    });
  }
});
...
router.post("/", requireAdmin, upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      const publicPath = path.join(uploadDir, req.file.filename);
    ...
    log(
      "Create collection error details:",
