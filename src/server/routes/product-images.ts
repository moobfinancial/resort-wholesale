...
router.delete("/:productId/images/:imageId", optionalAuth, async (req, res) => {
  try {
    ...
    console.log(
      `Attempting to delete image with id: ${imageId} from product: ${productId}`
    );

    // Add validation for UUID formats
    if (!isValidUUID(productId) || !isValidUUID(imageId)) {
      console.error(
        `Invalid UUID format - productId: ${productId}, imageId: ${imageId}`
      );
      return res.status(400).json({
        status: "error",
    ...
    if (!image) {
      ...
      return res.status(404).json({
        ...
        message: "Image not found",
        details: anyImage
          ? "Image exists but may belong to a different product"
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Uploading image for variant: ${variantId} of product: ${productId}`
      );
      ...
      console.log(
        `Variant image created with ID: ${newImage.id}, URL: ${imageUrl}`
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Deleting image ${imageId} for product ${productId} variant ${variantId}`
      );
      ...
      if (
        typedImage.productId !== productId ||
        typedImage.variantId !== variantId
      ) {
