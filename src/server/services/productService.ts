...
export const productService = {
  async createProduct(
    ...
    try {
      ...
      const generatedSku =
        productRest.sku?.trim?.() ||
        `${productRest.name.replace(/\s+/g, "-").toLowerCase()}-${crypto
          .randomUUID()
          .slice(0, 8)}`;
  ...
  async getNewArrivals(): Promise<ApiResponse<{ items: any[] }>> {
    try {
      ...
      return {
        ...
        data: { items: [] },
        message:
          error instanceof Error ? error.message : "Failed to get new arrivals",
        details: error instanceof Error ? error.stack : undefined,
      };
    }
  },
  ...
  async deleteProduct(id: string): Promise<ApiResponse<{ item: any }>> {
    try {
      ...
      // First get the product with basic includes
      const existingProduct = (await prisma.product.findUnique({
        where: { id },
        include: {
          Variants: {
      ...
      console.log(
        `Preparing to delete product ${id} with name: ${existingProduct.name}`
      );
      ...
      if (
        ...
        console.log(
          `Product has ${existingProduct.ProductImages.length} additional images`
        );
      ...
      if (existingProduct.Variants && existingProduct.Variants.length > 0) {
        ...
        existingProduct.Variants.forEach((variant: any) => {
          ...
          if (variant.images && variant.images.length > 0) {
            console.log(
              `Variant ${variant.id} has ${variant.images.length} images`
            );
            variant.images.forEach((image: any) => {
              if (image.url) {
                const variantAdditionalImagePaths = normalizeImagePath(
                  image.url
                );
      ...
      const deletedProduct = await prisma.$transaction(async (tx) => {
        ...
        if (existingProduct.Variants && existingProduct.Variants.length > 0) {
          for (const variant of existingProduct.Variants) {
            if (variant.images && variant.images.length > 0) {
              console.log(
                `Deleting ${variant.images.length} images for variant ${variant.id}`
              );
        ...
        if (
          ...
          console.log(
            `Deleting ${existingProduct.ProductImages.length} images for product ${id}`
          );
          await tx.productImage.deleteMany({
            where: { productId: id },
          });
        ...
        if (existingProduct.Variants && existingProduct.Variants.length > 0) {
          console.log(
            `Deleting ${existingProduct.Variants.length} variants for product ${id}`
          );
        ...
        if (
          ...
          existingProduct.BulkPricing.length > 0
        ) {
          console.log(
            `Deleting ${existingProduct.BulkPricing.length} bulk pricing rules for product ${id}`
          );
        ...
        if (deletedCartItems.count > 0) {
          console.log(
            `Deleted ${deletedCartItems.count} cart items referencing product ${id}`
          );
        }
        ...
        try {
          ...
          if (deletedOrderItems.count > 0) {
            console.log(
              `Deleted ${deletedOrderItems.count} order items referencing product ${id}`
            );
          }
        } catch (e) {
          console.log(
            "Could not delete order items - they may be protected by foreign key constraints"
          );
        }
      ...
      if (imagePaths.length > 0) {
        ...
        console.log(
          `Successfully deleted ${deletedCount} files. Failed to delete ${failedCount} files.`
        );
      }
  ...
  async getPriceForQuantity(
    ...
    try {
      ...
      const applicableTier = bulkPricing.find(
        (tier: any) => quantity >= tier.minQuantity
      );
      ...
      console.error(
        `Error getting price for product ${productId} and quantity ${quantity}:`,
        error
      );
      return {
        status: "error",
