...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    const fetchVariantsExplicitly = async () => {
      ...
      try {
        ...
        const response = await frontendProductApi.getProductVariants(currentProduct.id);
        ...
        if (processedVariants.length > 0) {
          ...
          if (Object.keys(initialAttributes).length > 0) {
            const initialVariant = processedVariants.find((variant: any) => 
              Object.entries(initialAttributes).every(([key, value]) => 
  ...
  useEffect(() => {
    ...
    const fetchProductImages = async () => {
      ...
      try {
        ...
        const response = await api.get<any>(`products/${currentProduct.id}/images`);
  ...
  useEffect(() => {
    ...
    const fetchVariantImages = async () => {
      ...
      try {
        ...
        const response = await api.get<any>(`products/variants/${selectedVariant.id}/images`);
        console.log(`Raw variant images response for variant ${selectedVariant.id}:`, response);
        ...
        if (response && response.status === 'success' && response.data) {
          ...
          console.log(`Successfully processed ${images.length} images for variant ${selectedVariant.id}`);
          setVariantImages(images);
        } else {
          console.warn(`Invalid response format or error status for variant ${selectedVariant.id} images:`, response);
          message.error('Failed to load variant images');
          setVariantImages([]);
        }
      } catch (error) {
        console.error(`Error fetching images for variant ${selectedVariant.id}:`, error);
        message.error('Failed to load variant images');
        setVariantImages([]);
      } finally {
        setImagesLoading(false);
  ...
  useEffect(() => {
    ...
    console.log('Finding variant based on selected attributes:', selectedAttributes);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (selectedVariant) {
          await cartStore.addItem(currentProduct.id, quantity, selectedVariant.id);
        } else {
        ...
        if (selectedVariant) {
          await guestCartStore.addItem(currentProduct.id, quantity, selectedVariant.id);
        } else {
        ...
        <div className="relative">
          {imagesLoading ? (
            ...
            <InlineProductImageGallery
              ...
              defaultImage={currentProduct?.imageUrl || '/images/products/placeholder.svg'}
          ...
          {variantsLoading ? (
            ...
            productVariants.length > 0 && (
              <div className="mb-8">
                ...
                {Object.entries(selectedAttributes).map(([attributeType, selectedValue]) => (
                  <div key={attributeType} className="mb-4">
                    ...
                    <Select
                      ...
                      value={selectedValue}
                    >
                      {Array.from(new Set(productVariants
                        .filter(variant => variant.attributes)
                        .map(variant => variant.attributes[attributeType])
                      )).map(value => (
